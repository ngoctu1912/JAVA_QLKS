...
public class PhongBUS {
    ...
    public List<PhongDTO> getAllAvailableRooms() throws IllegalStateException {
        try {
            ...
            if (rooms.isEmpty()) {
                throw new IllegalStateException("Không có phòng khả dụng nào!");
            }
            return rooms;
        } catch (Exception e) {
            throw new IllegalStateException("Lỗi khi lấy danh sách phòng khả dụng: " + e.getMessage(), e);
        }
    }
    ...
    public List<PhongDTO> getAvailableRooms(Date checkInDate, Date checkOutDate, String guestType) throws IllegalArgumentException, IllegalStateException {
        ...
        try {
            ...
            if (rooms.isEmpty()) {
                throw new IllegalStateException("Không tìm thấy phòng trống phù hợp trong khoảng thời gian từ " +
                        new java.text.SimpleDateFormat("dd-MM-yyyy").format(checkInDate) + " đến " +
                        new java.text.SimpleDateFormat("dd-MM-yyyy").format(checkOutDate) + "!");
            }
            return rooms;
        } catch (Exception e) {
