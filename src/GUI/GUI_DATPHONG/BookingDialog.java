...
public class BookingDialog {
    ...
    private final boolean isAdminContext;
    ...
    public BookingDialog(JComponent parent, List<String> selectedRoomIds, String selectedRoomsText,
            Date checkInDate, Date checkOutDate, boolean isAdminContext) {
        this.parent = parent;
    ...
    public void showDialog() {
        ...
        confirmButton.addActionListener(e -> {
            ...
            if (!phone.matches("\\d{8,15}")) {
                JOptionPane.showMessageDialog(parent, "Số điện thoại phải là số và có 8-15 chữ số!",
                        "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }

            // Tìm khách hàng trong cơ sở dữ liệu
            KhachHangDTO khachHang = khachHangBUS.findByNameAndPhone(name, phone);
            if (khachHang == null) {
                JOptionPane.showMessageDialog(parent,
                        "Khách hàng với tên '" + name + "' và số điện thoại '" + phone + "' không tồn tại trong hệ thống!",
                        "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }

            int maKH = khachHang.getMaKhachHang();
