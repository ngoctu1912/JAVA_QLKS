...
public class ButtonCustom extends JButton {
    ...
    private void setIconFromSVG(String linkIcon) {
        if (linkIcon != null && !linkIcon.isEmpty()) {
            try {
                java.net.URL imgURL = getClass().getResource(linkIcon);
                if (imgURL != null) {
                    FlatSVGIcon svgIcon = new FlatSVGIcon(imgURL); // Kích thước 24x24
                    this.setIcon(svgIcon);
                } else {
                    System.err.println("SVG icon not found: " + linkIcon);
                }
            } catch (Exception e) {
                System.err.println("Error loading SVG icon: " + linkIcon);
                e.printStackTrace();
            }
        }
    }
